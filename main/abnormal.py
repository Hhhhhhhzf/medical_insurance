            ...
            for org_id, org_records in p_records.groupby(by='AKB020_1'):
                ...
                for settle_id, settle_records in org_records.groupby(by='AKC190'):
                    ...
                    if one_money > 0:
                        ...
                        hospital_parameters[i][pid][org_id+'_'+settle_id] = [one_days, one_liezhi_money, one_med_fee/one_money, one_exp_fee/one_money]
    ...
    for i, hospital_type in enumerate(hospital_types):
        ...
        for pid, p_records in hospital_data.groupby(by='AAC001'):
            ...
            if all_count > 0:
                ...
                hospital_parameters[i][pid]['org_count'] = org_count
...
def main():
    ...
    for i, hospital_type in enumerate(hospital_types):
        ...
        for pid, parameter in hospital_parameter.items():
            for org_settle, line in parameter.items():
                ...
                if line[0] <= thresholds.loc[i]['th_one_days'] and line[1] <= thresholds.loc[i]['th_one_liezhi_money'] and line[2] <= thresholds.loc[i]['th_one_med_p_low'] and line[3] >= thresholds.loc[i]['th_one_exp_p']:
                    dfs_detail.append([org_settle.split('_')[0], org_settle.split('_')[1]])
                # 住院时间相对短、药占比特别高
                if line[0] <= thresholds.loc[i]['th_one_days'] and line[2] >= thresholds.loc[i]['th_one_med_p_high']:
                    dfs_detail.append([org_settle.split('_')[0], org_settle.split('_')[1]])
    ...
    #     for pid, parameter in hospital_parameter.items():
    #         # 年龄大、次数多、机构单一
    #         if parameter['org_count'] <= thresholds[i]['th_org_count'] and parameter['all_count'] >= thresholds[i]['th_all_count']:
    ...
    result_info_detail.to_excel(os.path.join(output_path, 'DM_WARNING_HOSPITAL_DETAIL_1.xlsx'), index=False)
