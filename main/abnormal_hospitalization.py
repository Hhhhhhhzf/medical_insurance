...
def preprocess_old(hospital_types):
    ...
    for i, hospital_type in enumerate(hospital_types):
        ...
        if cnt > 0:
            ...
            thresholds.loc[i, 'th_avg_money'] = min(np.quantile(avg_money_list, 0.05), 300)
            thresholds.loc[i, 'th_one_money'] = np.quantile(one_money_list, 0.05)
            thresholds.loc[i, 'th_one_days'] = 60
        df_person_records = pd.DataFrame(person_records, columns=record_head)
