...
TEST_THRESHOLD = 100  # 最大限制
CHRONIC_NAMES = set(['哮喘', '高血压', '糖尿病', '阿尔茨海默病', '帕金森', '慢性肾脏', '肾病综合征', '慢性肾', '哮喘', '肺结核',
                     '透析', '慢性阻塞性肺病', '精神分裂症', '高脂血症', '前列腺增生', '慢性肝炎', '肝病', '脂肪肝', '肝炎',
                     '类风湿性关节炎'])


def is_chronic(disease_name):
    """
    判断是否是慢性疾病
    :param disease_name:
    :return:
...
def preprocess(file_name, year):
    ...
    record_head = ['p_id', 'p_name', 'age', 'org_count', 'in_time', 'all_count', 'liezhi_money', 'med_p', 'exp_p',
                   'org_codes', 'settle_ids', 'avg_money', 'one_days', 'max_one_test_count', 'max_one_test_name', 'type', 'flag']
    # 个人单次住院数据汇总
    person_records = []
    ...
    code2name = dict()
    # 按人进行分组
    person_bar = tqdm(hospital_data.groupby(by='PC_ID'))
    for pid, p_records in person_bar:
        temp_records = []
        # 按个人就医机构进行分组
        time_set = set()
        ...
        for org_id, org_records in p_records.groupby(by='ORG_ID'):
            ...
            for settle_id, settle_records in org_records.groupby(by='SETTLE_ID'):
                ...
                invalid_flag = 0
                one_test_count = dict()
                disease_name = line0['OUT_DES_NAME']
                if disease_name == '高血压':
                    pass
                if not is_chronic(disease_name):
                    for j in range(len(settle_records)):
                        record = settle_records.iloc[j]
                        catalog_code = record['CATALOG_CODE']
                        code2name[catalog_code] = record['CATALOG_NAME']
                    ...
                    max_one_test_name = '无化验数据'
                    if len(one_test_count) > 0:
                        for code, code_count in one_test_count.items():
                            if code_count > max_one_test_count:
                                max_one_test_count = code_count
                                max_one_test_name = code2name[code]
                    # 单次住院有效
                    all_count += 1
                    all_money += one_money
                    ...
                    person_record = [pid, line0['NAME'], int(line0['AGE']), 1, str(line0['TIME']), 1, one_liezhi_money,
                                     ...
                                     one_days, max_one_test_count, max_one_test_name, '', invalid_flag]
...
def preprocess_old(file_name, year):
    ...
    record_head = ['p_id', 'p_name', 'age', 'org_count', 'all_count', 'liezhi_money', 'med_p', 'exp_p', 'org_codes',
                   'settle_ids', 'type', 'one_money', 'avg_money', 'one_days', 'max_one_test_count', 'max_one_test_name',
                   'flag']
    hospital_data = pd.read_pickle(os.path.join(pickle_path, file_name))
    cnt = 0
    all_days_list = []
            ...
            for settle_id, settle_records in org_records.groupby(by='SETTLE_ID'):
                ...
                if not is_chronic(disease_name):
                    ...
                    person_record = [pid, line0['NAME'], int(line0['AGE']), 1, 1, one_liezhi_money,
                                     ...
                                     one_money/one_days, one_days, max_one_test_count, max_one_test_name, invalid_flag]
                    temp_records.append(person_record)
        if all_count > 0:
            cnt += 1
            all_count_list.append(all_count)
...
def main(year):
    ...
    ['p_id', 'p_name', 'age', 'org_count', 'all_count', 'liezhi_money', 'med_p', 'exp_p', 'org_codes',
     'settle_ids', 'type', 'one_money', 'avg_money', 'one_days', 'max_one_test_count', 'max_one_test_name', 'flag']
    columns_index1 = ['主键', '就诊人', '就诊人名称', '年龄', '住院机构数量', '住院次数', '住院次数阈值', '药占比', '药占比阈值', '检查比率', '检查比率阈值',
                      '机构编码逗号分隔', '类别', '住院列支总费用', '住院列支总费用阈值', '住院时长', '住院时长阈值',
                      '日平均费用', '日平均费用阈值', '最大单项化验次数', '最大单项化验次数阈值', '最大单项化验名称']
    ...
    for i, hospital_type in enumerate(['all']):
        ...
        for j in range(len(examination_type)):
            ...
            result.append([index1, row['p_id'], row['p_name'], row['age'], row['org_count'], row['all_count'], threshold['th_all_count'],
                           ...
                           row['max_one_test_count'], threshold['th_one_test_count'], row['max_one_test_name']])
        ...
        over_test_type.loc[:, 'type'] = '过度/虚记化验'
...
def detail_export(raw_data, output_detail):
    ...
    :return:
    """
    output = pd.DataFrame([], columns=['外键'] + list(raw_data.columns))
    for i in tqdm(range(len(output_detail))):
        ...
        output = pd.concat([output, details_data])

    header = ['外键', '定点机构编码', '定点机构名称', '证件号码', '姓名', '性别', '参保单位名称', '参保类型', '年龄', '单据号',
              '收费项目类别', '金额', '医保范围费用', '医保目录编码', '医保目录名称', '数量', '结算时间', '住院天数', '出院疾病名称',
              '出院疾病诊断编码', '费用发生时间', '入院时间', '出院时间', '费用类别', '医疗类别']
    output.columns = pd.Series(header)
    print('ready export')
    write2excel(output, output_path, 'ZHUYUAN_DETAIL.xlsx')
    print('export over!')
    # - - -- - -- --- -- - - - - - - -- - -
    # 测试内容，用后删除
    # d_names = set(output['出院疾病名称'])
    # for name in d_names:
    #     print(name, 'is c:', is_chronic(name))
