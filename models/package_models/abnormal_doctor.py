...
def is_leap_year(year_):
    return year_ % 400 == 0 or (year_ % 100 != 0 and year_ % 4 == 0)
...
def total_seconds(date):
    ...
    cnt_second = (np.sum(month_days[:int(m)]) + int(d) - 1) * 24 * 60 * 60 + int(hour) * 60 * 60 + int(minute) * 60 + int(second)
    return cnt_second


def get_yichang_doctor(year_):
    db = connor()
    cursor = db.cursor()
    ...
    sql = """SELECT
            ...
            WHERE
                c.AAA027 = '330183' 
                AND substr( b.AKC002, 1, 4 ) = %s
                AND b.BKC903 != 1""" % year_
    cursor.execute(sql)
    while True:
    ...
    output.to_excel('../data/model_shangxian/医师违规%s_final.xlsx' % year_, index=False)
    return output


def get_doctor_mingxi(year_):
    data = pd.read_excel('../data/model_shangxian/医师违规%s_final.xlsx' % year_, encoding='utf-8').values
    doctor_ids = dict()
    ...
    output_data.to_excel('../data/model_shangxian/医师违规明细%s_final.xlsx' % year_, index=False)
    return output_data


def fetch_now_data(year_):
    main_table_, detail_count_ = dict(), 0
    db = connor()
    cursor = db.cursor()
    sql_line = """select * from DM_WARING_PLACES a WHERE a.YEAR = %s""" % year_
    ...
    while True:
        ...
        if len(rows) == 0:
            break
        for row in rows:
            main_table_[row[2]] = row
    if len(main_table_) > 0:
        sql_line = """select * from DM_WARING_PLACES_DETAIL a WHERE a.YEAR = %s""" % year_
        cursor.execute(sql_line)
        while True:
            rows = cursor.fetchmany(1000)
            if len(rows) == 0:
                break
            detail_count_ += len(rows)
    return main_table_, detail_count_


# 导入到数据库中
def import_to_db(data_frame, table, start_index):
    pass
...
if __name__ == '__main__':
    ...
    main_table, detail_count_ = fetch_now_data(year)
    # 获取违规医师，最新数据
    main_data = get_yichang_doctor(year)
    detail_data = get_doctor_mingxi(year)
    main_ready, detail_ready, main_index, detail_index = None, None, 0, 0
    if len(main_table) == 0:  # 主表中存在记录
        main_ready = main_data
        detail_ready = detail_data
    else:
        main_index, detail_index = len(main_table), detail_count_
